...
class GitHubReviewer:
    ...
    def __init__(
        ...
        Args:
            ...
            dynamodb_table:          DynamoDB Table resource for caching
        """
        if not github_app_id or not github_private_key_path:
            raise ValueError("GITHUB_APP_ID and GITHUB_PRIVATE_KEY_PATH are required")
        ...
        try:
            ...
            raise ValueError(f"GitHub private key not found at {github_private_key_path}")

        self.bedrock_client = bedrock_client
        self.model_id = model_id
        self.table = dynamodb_table
        self.token = "" # Dynamically generated per installation or request
    ...
    def handle_pr_event(self, payload: dict) -> dict:
        ...
        if not installation_id:
            return {"status": "error", "reason": "Missing installation ID. Is this configured as a GitHub App webhook?"}
        ...
        try:
            ...
            encoded_jwt = jwt.encode(payload_jwt, self.private_key, algorithm="RS256")
            headers = {
            ...
            if r.status_code != 201:
                return {"status": "error", "reason": f"GitHub App Auth failed: {r.status_code} {r.text}"}

            self.token = r.json()["token"]
        except Exception as e:
            logger.error("Failed to get installation token: %s", e)
            return {"status": "error", "reason": f"GitHub App Auth failed: {e}"}
